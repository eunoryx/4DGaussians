...
import copy
from adaptive_density_integration import AdaptiveDensityManager  # <-- ADDED
...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    # ADDED: initialise adaptive density manager for fine stage
    adaptive_manager = None
    if stage == "fine":
        adaptive_manager = AdaptiveDensityManager(output_dir=scene.model_path)
        print("[Adaptive Density] Manager initialised for fine stage.")
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0 and gaussians.get_xyz.shape[0] < 360000:
                    ...
                    # ADDED: use adaptive densification in fine stage, fallback to original in coarse
                    if adaptive_manager is not None:
                        # Update gradient accumulation
                        adaptive_manager.update_gradients(viewspace_point_tensor_grad, visibility_filter)

                        # Get adaptive masks
                        selected_pts_mask, prune_mask, stats = adaptive_manager.adaptive_densify_and_prune(
                            gaussians, opt, scene.cameras_extent, iteration, stage,
                            densify_threshold, opacity_threshold
                        )

                        # Apply densification using selected mask
                        gaussians.densify_and_clone(selected_pts_mask, densify_threshold, scene.cameras_extent)
                        gaussians.densify_and_split(selected_pts_mask, densify_threshold, scene.cameras_extent)
                        ...
                        if iteration % 1000 == 0:
                            adaptive_manager.visualize(gaussians, iteration)
                    else:
                        # Original densification for coarse stage
                        gaussians.densify(densify_threshold, opacity_threshold, scene.cameras_extent, size_threshold, 5, 5, scene.model_path, iteration, stage)

                if iteration > opt.pruning_from_iter and iteration % opt.pruning_interval == 0 and gaussians.get_xyz.shape[0] > 200000:
                    size_threshold = 20 if iteration > opt.opacity_reset_interval else None
                    if adaptive_manager is None:
                        # Only run original pruning in coarse stage
    ...
    # ADDED: save summary at end of fine stage
    if adaptive_manager is not None:
        adaptive_manager.create_summary()
